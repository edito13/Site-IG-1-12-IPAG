    ...
    form.addEventListener("submit", (e)=>{
        ...
        try {
            ...
            $.ajax({
                ...
                // Se inseriu no banco, então da um refresh na página
        ...
        $.ajax({
            ...
            // Formatando a data recebida do banco
                ...
                if(dataOficial == date.toLocaleDateString()){
                    ...
                    // Se já passaram-se uma ou mais, mostra a/s hora/s que passaram-se    
                            ...
                            // Senão mostra a  hora
                                ...
                                else if(minutoActual == minutoComentario) resposta = `Comentado há ${horaIdeal} hora.`
                                else{
                                    if(minutoIdeal >= 60) resposta = `Comentado há ${horaIdeal} hora.`
                                    else resposta = `Comentado há ${horaIdeal} hora e ${minutoIdeal} minutos.`
                                }
                            }else{
                            // Se já fez mais de uma hora mas ainda não chegou na hora proxima, pork o minuto ainda não chegou, então essa hora não conta, --1
                                if(minutoActual < minutoComentario){
                                    // Se passou-se uma ou mais horas e o minuto que passou-se é maior ou igual 60, então mostra só a hora 
                                    if(minutoIdeal >= 60){
                                        // Se passou-se duas horas, menos uma fica uma hora e não horas
                                        if(horaIdeal == 2) resposta = `Comentado há ${--horaIdeal} hora.`
                                        // Senão são horas mesmo
                                        else resposta = `Comentado há ${--horaIdeal} horas.`
                                    }
                                    //Senão mostra a hora e os minutos
                                    else{
                                        // Se passou-se duas horas, menos uma fica uma hora e não horas
                                        if(horaIdeal == 2) resposta = `Comentado há ${--horaIdeal} hora e ${minutoIdeal} minutos.`
                                        // Senão são horas mesmo
                                        else resposta = `Comentado há ${--horaIdeal} horas e ${minutoIdeal} minutos.`
                                    }
                            // Se não, então mostra simplismente as horas
                                }else{
                                    if(minutoIdeal >= 60) resposta = `Comentado há ${horaIdeal} horas.`
                                    else resposta = `Comentado há ${horaIdeal} horas e ${minutoIdeal} minutos.`
                    ...
                    if(diaIdeal == 1){
                        ...
                        if(horaActual < horaComentario){
                            ...
                            if(minutoActual < minutoComentario){
                                ...
                                if(horaIdeal >= 24) resposta = `Comentado há ${diaIdeal} dia.`
                                else resposta = `Comentado há ${diaIdeal} dia e ${horaIdeal} horas.`
                            }
                        }else{
                            if(horaIdeal >= 24) resposta = `Comentado há ${diaIdeal} dia.`
                            else resposta = `Comentado há ${diaIdeal} dia e ${horaIdeal} horas.`
                ...
                // Se já passaram-se dias, mostra os dias que passaram-se
                        ...
                        if(horaActual >= horaComentario && minutoActual >= minutoComentario){
                            if(horaIdeal >= 24) resposta = `Comentado há ${diaIdeal} dias.`
                            else resposta = `Comentado há ${diaIdeal} dias e ${horaIdeal} horas.`
                        }else{
                            if(horaIdeal >= 24){
                                if(diaIdeal == 2) resposta = `Comentado há ${--diaIdeal} dia.`
                                else resposta = `Comentado há ${--diaIdeal} dias.`
                            }else{
                                if(diaIdeal == 2) resposta = `Comentado há ${--diaIdeal} dia e ${horaIdeal} horas.`
                                else resposta = `Comentado há ${--diaIdeal} dias e ${horaIdeal} horas.`
                            }
                        }
                    }
                ...
                }else if(anoActual == anoComentario && mesActual > mesComentario && (diaActual == diaComentario || diaActual > diaComentario) && (horaActual == horaComentario || horaActual > horaComentario) && (minutoActual == minutoComentario || minutoActual > minutoComentario)){
